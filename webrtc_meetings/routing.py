...
def mark_present(message):
    ...
    wlogger.info('msg ip: %s, port: %s ,channel:%s' % (ip, port,message.reply_channel.name))
    details=get_request_params(message['query_string'])
    a = Attendant(ip=ip, port=port, channel_name=message.reply_channel.name,
                  details=details)
    #isAttendantExists(details['name'])
    store.hset(ONLINE, message.reply_channel.name, Attendant.dumps(a))
    sendId(message.reply_channel.name)

def sendId(attendantId):
    if store.hexists(ONLINE, attendantId):
        wlogger.info('start')
        info={}
        wlogger.info('starts')
        info['id']=attendantId+''
        wlogger.info('startss')
        #info={'id':''+attendantId}
        #wlogger.info('info %s ,type %s' % (info,type(json.dumps(info))))
        Channel(attendantId).send({'text':json.dumps(info)})
        wlogger.info('Msg sent')
        
def isAttendantExists(attendantName):
    attendants = []
    attendant_datas = store.hvals(ONLINE)
    map(lambda data: attendants.append(Attendant.loads(data)), attendant_datas)
    for attendant in attendants:
        if(attendant['details']['name']==attendantName):
	    closeExist(attendant['id'])
 	    break	

def closeExist(attendantId):
    store.hdel(ONLINE,attendantId)	
